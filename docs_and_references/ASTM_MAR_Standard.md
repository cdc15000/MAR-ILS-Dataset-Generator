...
**5.5** This test method is structured to support incorporation by normative reference into external performance standards. **[Rev 04]** Acceptance criteria based on ΔAUC values are established by the incorporating authority, not by this standard. The intended path is incorporation by reference into a post-publication Amendment to IEC 60601-2-44 Ed. 4 (§203.6.7.101.1), with regulatory authorities (e.g., FDA) issuing guidance on acceptance thresholds. This standard defines only the measurement method.
...
> **[Rev 05] Statistical basis for N = 40 (provisional; pilot is definitive).** Evaluated on the reference characterization (N = 40, σ_internal = 15; the LI-MAR control run recorded in `cho_results.json`), the reported **ΔAUC test result** meets both §17.1.6 targets:
>
> | Quantity (ΔAUC test result, N = 40) | Value | §17.1.6 target |
> |---|---|---|
> | Sampling SD(ΔAUC) (from the paired ΔAUC bootstrap 95% CI) | ≈ 0.038 | ≤ 0.05 ✓ |
> | Bias of ΔAUC (resubstitution − hold-out) | ≈ 0.004 | ≤ 0.02 ✓ |
>
> As an independent cross-check, the analytic Hanley–McNeil standard error for a single AUC at AUC_noMAR = 0.8294 (n = m = 40) is ≈ 0.046 — the same order as the bootstrap SD.
>
> **Important distinction.** The *per-condition* AUC resubstitution−hold-out bias is large (≈ 0.12 for both noMAR and MAR) — the intrinsic small-sample optimism of a 10-channel plug-in Hotelling observer — but it is **common-mode and cancels** in ΔAUC (≈ 0.12 − 0.12 ≈ 0.004). Because ΔAUC is the reported test result, the §17.1.6(c) bias gate is met; gating the per-condition AUC bias instead would be unachievable at N = 40 for reasons unrelated to test-result accuracy (see the clarification in §17.1.6(c)).
>
> These figures rest on a single operating point (the LI-MAR control); they **support** N = 40 as adequate for the ΔAUC targets but do not settle precision — the pilot precision study (§17.1.6) remains definitive. An informational Monte Carlo of the estimator's ≈ 1/√N scaling is provided in `research/power_analysis_n40.py` (illustrative only; its operating-point calibration is approximate and its absolute figures are not relied upon here).
...
**17.1.6** Pilot precision data requirement: Before this standard proceeds to first ASTM ballot, the sponsoring subcommittee shall provide pilot precision data from a minimum of 3 laboratories demonstrating that: **(a) [Rev 05]** the test pipeline resolves a *known, signed* ΔAUC of the expected sign and magnitude — i.e., across laboratories the measured ΔAUC for a designated control algorithm agrees in sign and is statistically distinguishable from zero (a negative control such as the LI-MAR reference designated in the note below satisfies this; a positive control may be used additionally but is not required); (b) the within-laboratory standard deviation of ΔAUC does not exceed 0.05 AUC units; and **(c) [Rev 05]** the bias of the **ΔAUC test result** (resubstitution minus hold-out) does not exceed 0.02 AUC units at N = 40 realizations. This gate applies to ΔAUC, not to the per-condition AUC: the per-condition resubstitution−hold-out optimism of a 10-channel plug-in Hotelling observer is intrinsically large (≈ 0.12 at N = 40, reported as a diagnostic per §16(k)) but is common-mode and cancels in ΔAUC (the reference characterization gives a ΔAUC bias ≈ 0.004); gating the per-condition AUC bias at 0.02 would be unachievable for reasons unrelated to the accuracy of the reported test result.
